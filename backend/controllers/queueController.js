...
export const getLiveSimulation = async (req, res) => {
    try {
        ...
        if (statsRes.error) throw new Error(`Stats query failed: ${statsRes.error.message}`);
        if (hospitalRes.error) throw new Error(`Hospital query failed: ${hospitalRes.error.message}`);
        if (doctorsRes.error) throw new Error(`Doctors query failed: ${doctorsRes.error.message}`);
        ...
        console.error('Error in getLiveSimulation:', error);
        res.status(500).json({ error: error.message || 'Failed to get live simulation' });
    }
};